...
| `npm run test:functions` | Node's built-in test runner (`node --test`), via `npm --prefix functions test` | The Cloud Functions matching logic under `functions/src/matching/`: deterministic scoring, input hashing, and explanation generation (including its stub/fallback path) — no emulator, no network call, no Admin SDK initialization. Not part of `npm test` (see below) since it lives in a separate `functions/` package with its own `node_modules`; run it directly, or via this script. |
